...
import { PREFIX_WEB_STUDENT } from '../interfaces/constants'
...
export class LoginOrkestraComponent implements OnInit {
  ...
  public onLogin (): void {
    if (this.agentID_default.length > 2 && this.room_default.length > 2) {
      this.errorMsg = ''
      this.orkestraService.connect(PREFIX_WEB_STUDENT + this.agentID_default, this.room_default)
